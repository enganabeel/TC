            ...
            for slip in lines.slip_ids:
                if lines.slip_ids:
                    if slip.struct_id.id == used_struct[0]:
                        ...
                        sheet.write(e_name, 1, slip.employee_id.employee_no, format3)
...
class PayrollBankReport(models.AbstractModel):
    _name = 'report.xlsx_payroll_report.xlsx_payroll_bank_report'
    _inherit = 'report.report_xlsx.abstract'
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        for used_struct in used_structures:
            ...
            # Fetch available salary rules:
            for item in lines.slip_ids.struct_id.rule_ids:
                if item.struct_id.id == used_struct[0] and item.category_id.code=='NET':
                    col_title = ''
                    row = [None, None, None, None, None]
                    row[0] = col_no
                    row[1] = item.code
