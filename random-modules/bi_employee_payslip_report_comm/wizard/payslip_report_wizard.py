...
class EmpPayslipReport(models.TransientModel):
    ...
    def employee_payslip_xls(self):
        if self.file_type == 'pdf':
            ...
            return self.env.ref('bi_employee_payslip_report_comm.action_report_export_emp_payslip').report_action(self,data=datas)
            ...
            worksheet.merge_range(3, 3, 4, 3, 'Account number', cell_wrap_format_bold)
            ...
            for line in lines:
                ...
                if 'Allowance' in line.keys():
                    val = len(line.get('Allowance'))
                    values  = line.get('Allowance')
                ...
                if 'Deduction' in line.keys():
                    val = len(line.get('Deduction'))
                    values  = line.get('Deduction')
            ...
            for payslip in payslip_ids:
                ...
                values['Account number'] = payslip.employee_id.bank_account_id.acc_number or '',
                ...
                for i , j in lable.items():
                    if i not in ['no', 'payslip_ref', 'employee', 'account number', 'period']:
            ...
            for value in main:
                col = 0
                row = row
                list = ['NO', 'Payslip_Ref', 'Employee', 'Account number', 'Period', 'Basic', 'Allowance', 'Gross', 'Deduction', 'Net']
                for l in list:
                    if l in value.keys():
                        data  = value.get(l)
                        for r in data:
            ...
            #For Get the Total
            total_row = end_row + 1
            list = ['Basic', 'Allowance', 'Gross', 'Deduction', 'Net']
            coln = 5
            for l in list:
                lst = []
                for mn  in main:
