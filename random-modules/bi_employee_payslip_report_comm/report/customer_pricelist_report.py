...
class ExportCustomerPricelistReport(models.AbstractModel):
	...
	def _get_payslip_details(self, payslip_ids):
		...
		d_lst = []
		n_lst = []
		print(payslip_ids.ids,'payslip_ids')
		print(self,'self')
		list_ids_emp = payslip_ids.ids
		emp_names = self.env['hr.payslip'].search([('employee_id','in',list_ids_emp)])
		print(emp_names,'emp_names')
		...
		for payslip in payslip_ids:
			...
			if lable.get('test'):
				values['Tes'] = payslip.employee_id.bank_account_id.acc_number or '',
			if lable.get('period'):
			...
			for i , j in lable.items():
				if i not in ['no', 'payslip_ref', 'employee', 'designation', 'period','test']:
