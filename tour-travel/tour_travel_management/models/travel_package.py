        ...
        for package in self:
            if package.arrival_date and package.return_date:
                ...
                if package.arrival_date > package.return_date:
                    raise ValidationError(_('''Return date should be greater than arrival date.'''))

    @api.constrains('state')
    def check_itinerary_ids(self):
        for rec in self:
            if not rec.itinerary_ids and rec.state == 'confirm':
                raise ValidationError(_('''Please add itinerary lines before confirm the package '%s'.'''% (rec.name,)))
